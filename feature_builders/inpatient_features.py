...
SCHEMA_PATH = Path(__file__).resolve().parent.parent / "models" / "model_b_anomaly" / "inpatient" / "feature_schema.json"

INPATIENT_HISTORICAL_FEATURES = [
    "CLAIM_COUNT",
    "UNIQUE_CLAIM_COUNT",
    "TOTAL_PAYMENT",
    "AVG_PAYMENT",
...
def load_inpatient_schema():
    with open(SCHEMA_PATH, "r") as f:
        schema = json.load(f)
    return schema["features"]


def build_inpatient_features(raw_claim: dict, provider_context: dict | None = None) -> pd.DataFrame:
    """
    Converts a raw INPATIENT claim + provider historical context into a single-row DataFrame
    matching the Inpatient Model B feature schema (exactly 62 features).
    ...
    provider_context : dict, optional
        Historical provider statistics for Inpatient model.

    Returns
    -------
    pd.DataFrame
        Single-row DataFrame with 62 features in exact schema order.
    """
    expected_features = load_inpatient_schema()

    if provider_context is None:
        provider_context = raw_claim.get("provider_context") or raw_claim.get("provider_history") or {}

    # Verify provider historical features
    missing_hist_keys = [key for key in INPATIENT_HISTORICAL_FEATURES if key not in provider_context and key not in raw_claim]
    if missing_hist_keys:
        raise ValueError(
            f"Missing required provider historical context for Inpatient Model B features: "
            f"The following keys must be explicitly provided in provider_context: {missing_hist_keys}"
        )
    ...
    # Claim payment & primary payer
    clm_pmt_amt = float(raw_claim.get("CLM_PMT_AMT", raw_claim.get("claim_payment", 0.0)))
    primary_payer_amt = float(raw_claim.get("NCH_PRMRY_PYR_CLM_PD_AMT", raw_claim.get("primary_payer_payment", 0.0)))
    utilization_days = float(raw_claim.get("CLM_UTLZTN_DAY_CNT", raw_claim.get("utilization_days", 1.0)))

    # Dates and duration
    claim_start = raw_claim.get("claim_start_date") or raw_claim.get("CLM_FROM_DT") or "2010-01-01"
    ...
    try:
        ...
        claim_year = float(raw_claim.get("CLAIM_YEAR", 2010.0))
    ...
    # Extract the 18 provider historical features from context
    hist_dict = {}
    for key in INPATIENT_HISTORICAL_FEATURES:
        val = provider_context.get(key, raw_claim.get(key))
        hist_dict[key] = float(val)
    ...
    if len(df.columns) != len(expected_features):
        raise ValueError(f"Inpatient feature count mismatch: expected {len(expected_features)}, got {len(df.columns)}")

    if list(df.columns) != expected_features:
        raise ValueError("Inpatient feature order/name mismatch against schema.")
